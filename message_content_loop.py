...
def message_content_loop(directory, queue, configurer):
    ...
    datafile = open( "/root/EmailParser/use-and-abuse/EmailParser/In-Process-JSONS/message_content.json", "w", encoding="utf-8" )  # opening json file for writing
    json.dump( mdict, datafile, indent=4,
               separators=(',', ': ') )  # printing data in nice format to file
    datafile.close()  # Closing File

    datafile = open( "/root/EmailParser/use-and-abuse/EmailParser/In-Process-JSONS/extra_links.json", "w", encoding="utf-8" )  # opening json file for writing
